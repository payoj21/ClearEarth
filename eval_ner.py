...
def join(c,p,gold,predict):
    ...
    fg = open(gold,'r')
    fp = open(predict, 'r')
    fw = open(p[:p.find('/results_test/')]+'/evaluate_script'+'.txt','a')
...
for p in path:
    ...
    for filename in os.listdir(p):
        if filename[0] != '.':
            ...
            join(i,p,p[:p.find('results_test/')]+'train_test/'+filename[:filename.find('.ner')]+'.train',p+filename)
            i += 1
